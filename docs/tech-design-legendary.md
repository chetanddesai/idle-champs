...
Implementation has started: `js/lib/scopeMatcher.js`, `js/lib/legendaryModel.js`, `js/lib/format.js`, `js/lib/heroImage.js`, `js/lib/legendaryDefsParser.js`, `js/lib/definitions.js`, `js/serverCalls.js`, `js/credentials.js`, `js/state.js`, `js/views/settings.js`, and `js/main.js` are landed with `node:test` coverage. The runtime now sources hero/effect/scope/favor definitions from a `localStorage` cache (`ic.definitions.cache`) populated by `getdefinitions` on every Refresh — see [Appendix B #14](#appendix-b-resolved-decisions--open-questions). The single bundled file is `data/definitions.hero-images.json` (Emmote's `ic_wiki` portrait slugs, Appendix B #9), refreshed manually via `npm run refresh-hero-images`. The remaining work is the two Legendary tab views and the Home dashboard.
...
| **FR-3** | On startup, the app loads `data/definitions.hero-images.json` synchronously, reads any cached game definitions from `ic.definitions.cache` (`localStorage`), checks `ic.credentials`, routes to `#/settings` (blocking all other routes) if absent, and once credentials exist runs `refreshAccount()` before rendering the routed view. A global header always shows "Last refreshed: Xm ago" with a refresh icon-button. **First-load UX:** if the user has credentials but no cached defs (e.g. brand-new browser), `refreshAccount()`'s `getdefinitions` leg populates the cache before the first Legendary render; if it isn't yet populated, the Legendary view shows a loading state per FR-7. | `js/main.js` *(new)* bootstrap + route guard | [Appendix B #3, #14](#appendix-b-resolved-decisions--open-questions) |
| **FR-4** | A single `refreshAccount()` function is the only entry point for hydrating account state. It calls `getuserdetails` exactly once and writes the entire legendary view's input data (`details.heroes`, `details.loot`, `details.legendary_details`, `details.stats.multiplayer_points`, `details.reset_currencies`, `details.instance_id`) through to the state store. **It then calls `getdefinitions` (no checksum, full filtered payload)** and persists the parsed result to `ic.definitions.cache` via `applyDefinitionsResponse()` (Appendix B #14). A `getdefinitions` failure is silent — it never blocks the user-state refresh. Both the Refresh button and post-mutation flows call this same function. | `js/state.js` `refreshAccount(deps)` + `js/serverCalls.js` `getUserDetails()` + `getDefinitions()` + `js/lib/definitions.js` `applyDefinitionsResponse()` | [Appendix B #2, #8, #14](#appendix-b-resolved-decisions--open-questions) |
...
| FR   | Requirement | Delivered by | Decision ref |
|------|-------------|--------------|--------------|
| **FR-7** | The Legendary view renders as a single card with two tabs (Forge Run, Reforge) sharing a DPS dropdown, a five-axis classification chip row (Race · Gender · Alignment · Damage-type · top ability scores), and a sticky Scales-of-Tiamat balance badge. The last-selected DPS (`ic.selected_dps_id`) and tab (`ic.legendary.activeTab`, default `forge-run`) are restored on mount. No tab content renders until a DPS is picked. **DPS dropdown ordering:** alphabetical by hero name; entries filtered to heroes the player owns (per `details.heroes`). Hero portraits in the dropdown and tiles resolve via `heroImageUrl(heroId, heroImages)` from `data/definitions.hero-images.json`, falling back to the first-word monogram when a slug is missing (Appendix B #9). **Loading state:** if neither `ic.definitions.cache` nor `data/definitions.hero-images.json` is ready, the view renders a "Refresh to load definitions" loader; the cache subscription (Appendix B #14) re-renders once `getdefinitions` lands. | `js/views/legendary/index.js` *(new)* — shell + tab switcher; `js/views/legendary/header.js` *(new)* — DPS selector + chip row + balance badge | PRD §3.2.3, §9 #16–#17; [Appendix B #9, #11, #14](#appendix-b-resolved-decisions--open-questions) |
| **FR-8** | For the selected DPS, the system computes — once per DPS change, memoized in session — a classification of every equipped legendary slot against the DPS: `{heroId, slotIndex, currentEffectId, affectsDps, heroRole, poolAffectingDps[], equippedLevel, requiredFavor}`. Classification uses `scopeMatcher.effectAffectsHero()` against the cached scope table. **Memoization shape (Appendix B #10):** the memo is a `Map<dpsHeroId, SlotClassificationMap>` held inside `js/views/legendary/index.js`; `state.subscribe('ic.userdetails', …)` and `state.subscribe('ic.definitions.cache', …)` callbacks both clear the entire map — the first when account state refreshes, the second when the runtime defs cache is rewritten by a Refresh (Appendix B #14). This keeps `classifySlots` itself pure and moves all cache-management concerns into the view layer. | `js/lib/legendaryModel.js` *(new)* `classifySlots(inputs)` — pure; consumes the cached `heroes` / `effects` / `scopes` (via `js/lib/definitions.js loadCachedDefs()`) + `details.legendary_details.legendary_items` + selected DPS id. | [Appendix B #4, #5a, #10, #14](#appendix-b-resolved-decisions--open-questions); PRD §3.2.2 |
...
| FR   | Requirement | Delivered by | Decision ref |
|------|-------------|--------------|--------------|
| **FR-14** | Classification failures (legendary effect with `scope.kind === 'unknown'`) render a banner above the Legendary card ("N effects couldn't be classified; please file an issue with the effect ID") but never block either tab. `js/lib/legendaryDefsParser.js` returns a `parsed.audit.unknown_scope_ids` list at runtime, and the maintainer-only `scripts/refresh-hero-images.js` runs the same parser as a sanity check and prints any unknown IDs to stderr — both serve as tripwires for new game content (Appendix B #14). | `js/lib/legendaryModel.js` (surfaces unknowns in its output); `js/views/legendary/index.js` (renders banner); `js/lib/legendaryDefsParser.js` (runtime audit list); `scripts/refresh-hero-images.js` (maintainer-side warning) | PRD §3.2.2, §12 success criteria |
...
| **NFR-4** | **Performance — Legendary runtime.** Selecting a DPS champion resolves the full slot classification (affecting / not affecting / reforge-candidate / empty) for all 173 heroes in < 100 ms on a mid-range mobile device, using only the runtime defs cache + bundled hero-portrait map — no network. | `classifySlots` is O(heroes × slots) with O(1) scope membership tests (via pre-indexed affecting-effect-id `Set` derived from `affectingEffectIds`). Result memoized in session keyed by `selected_dps_id` + userdetails version + defs-cache version (Appendix B #14). PRD §12 success criteria; FR-8. |
...
| **NFR-7** | **Credentials & privacy.** `user_id` and `hash` stored only in browser `localStorage`; transmitted only to `*.idlechampions.com`. No telemetry, analytics, or third-party CDN requests. Explicit "Clear all local data" action available. | `js/state.js` writes `ic.credentials` to `localStorage`; `js/serverCalls.js` is the only module that makes network requests and only to the master / play-server hosts. CSP `connect-src` meta-tag restricts `fetch` destinations as defense-in-depth. Settings panel surfaces "Clear all local data" (wipes credentials, cached account data, and favorites). PRD §2.2, §12 success criteria. |
| **NFR-8** | **Data freshness & caching.** `localStorage` is the source of truth for both account state and game definitions (`ic.definitions.cache`, see Appendix B #2 + #14). No TTL, no auto-expiry, no background polling — freshness is user-driven via the Refresh button (which fires both `getuserdetails` and `getdefinitions`). The bundled `data/definitions.hero-images.json` refreshes only via the `npm run refresh-hero-images` utility. | `state.js` writes through to `localStorage` on every set; views subscribe via pub/sub. `refreshAccount()` is the sole entry point for re-hydrating account state and the defs cache (FR-4). The bundled hero-portrait map is loaded once at view mount and not re-fetched at runtime. Appendix B #2, #8, #14; PRD §4.2. |
| **NFR-9** | **Graceful degradation for unknown scopes.** If the scope parser ever fails to classify a new legendary effect, the UI must continue to render the rest of the view and surface a banner citing the unknown effect IDs — never blocking the view or throwing. | `scopeMatcher.effectAffectsHero` returns `false` for `kind: 'unknown'` (already implemented). `legendaryModel` surfaces the list of unknown effect ids encountered in the current classification pass; `views/legendary/index.js` renders a dismissible banner above the card. `js/lib/legendaryDefsParser.js` exposes a runtime `audit.unknown_scope_ids` list and `scripts/refresh-hero-images.js` prints the same audit list to stderr — together they form the runtime + maintainer tripwires (Appendix B #14). PRD §3.2.2, §12 success criteria; FR-14. |
...
| **NFR-11** | **Zero runtime dependencies.** The shipped site has no runtime JavaScript dependencies. Typography (Inter / JetBrains Mono via Google Fonts) is the only external network request made at page load. Dev tooling (`node:test` for unit tests, `scripts/refresh-hero-images.js` utility) has zero external packages either. | `package.json` has no `dependencies` or `devDependencies` — `"type": "module"`, `test` and `refresh-hero-images` scripts only. PRD §2.1, §9 decision 7. |
...
|---|---|---|
| 1 | **Hosting platform is static GitHub Pages.** No server-side code, no serverless functions, no build step (PRD §2.1, §9 decision 1). | All API calls, retry loops, state management, and credential storage live client-side. Game definitions cache to `localStorage` at runtime (Appendix B #14); only `data/definitions.hero-images.json` is committed. Hero-image refresh tooling (`scripts/refresh-hero-images.js`) is a Node.js utility the developer runs locally — not a CI deploy step. |
...
| 3 | **Definitions data is large but changes only across game patches.** Full `getdefinitions` is ~1.5 MB; the four fields the Legendary view actually uses (`hero_defines`, `legendary_effect_defines`, derived `legendary-effect-scopes`, plus campaign-derived favor lookups) trim to **~85 KB stringified JSON** in the cache. | Definitions are fetched at runtime on every Refresh and cached in `localStorage` (`ic.definitions.cache`, Appendix B #14). The cache is parsed once into `js/lib/legendaryDefsParser.js`'s trimmed shapes via `applyDefinitionsResponse()`. The `checksum` parameter is deliberately **not** used (server-calls.md "Calling getDefinitions"); always-full-payload trades a few KB of bandwidth per Refresh for zero divergence risk between server state and client cache. |
| 4 | **`localStorage` budget is ~5 MB per origin.** | Trivial headroom: cached defs (~85 KB JSON) + cached `getuserdetails` response + credentials + a handful of UI-preference keys stay well under 1 MB total. No pressure to compress, chunk, or evict (Appendix B #2, #14). |
...
| 9 | **Legendary-effect scope taxonomy is derived, not authoritative.** The Idle Champions API does not expose structured targeting fields for legendary effects; the scope parser in `js/lib/legendaryDefsParser.js` derives `{kind, value}` from effect description text (PRD §3.2.2). A new game update could introduce an effect shape the parser doesn't recognize. | Parser tags unrecognized effects `{kind: 'unknown'}` and exposes their IDs as `parsed.audit.unknown_scope_ids` — a runtime tripwire (PRD §12 success criteria). Runtime `scopeMatcher` treats `unknown` as "does not affect anyone" (conservative). `legendaryModel` surfaces unknowns in its output so the view can render a dismissible banner (NFR-9, FR-14). The maintainer-only `npm run refresh-hero-images` invokes the same parser and prints the same audit list to stderr. |
...
flowchart TB
    ...
    subgraph Bundled["Bundled data (committed to repo)"]
        HeroImages[("data/definitions.hero-images.json")]
    end

    GHP -- "static asset serve" --> UI
    GHP -- "static asset serve" --> Bundled
    UI -- "read / write<br/>(ic.userdetails, ic.definitions.cache, ...)" --> LS
    LS -- "change events" --> UI
    UI -- "discovery (GET)" --> Master
    Master -- "play_server URL" --> UI
    UI -- "authenticated GET<br/>(getuserdetails, getdefinitions)<br/>V2 adds upgrade/craft/change" --> Play
...
flowchart LR
    ...
    subgraph Data["Data layer (impure)"]
        SC["serverCalls.js<br/>request, getPlayServerForDefinitions,<br/>getUserDetails, getDefinitions,<br/>upgrade/craft/change"]
        State["state.js<br/>get, set, subscribe,<br/>refreshAccount"]
        Defs["lib/definitions.js<br/>loadCachedDefs, loadHeroImages,<br/>applyDefinitionsResponse"]
    end
    ...
    subgraph Pure["Shared / pure (no I/O, no DOM)"]
        ...
        Parser["lib/legendaryDefsParser.js<br/>parseDefinitions"]
    ...
    class Scope,Model,Parser,Format,HeroImg pure
```

`lib/dom.js` and `lib/toast.js` are *almost* pure but touch the DOM, so they sit outside the green boundary. `scopeMatcher.js`, `legendaryModel.js`, `legendaryDefsParser.js`, `format.js`, and `heroImage.js` qualify for Node-only unit tests. `lib/definitions.js` is the runtime cache layer (impure — touches `localStorage` via `state.js` and reads the bundled hero-portrait JSON), and is the only module that imports `parseDefinitions`. **`lib/mutations.js` is deliberately absent from V1** — in-site mutations are deferred to V2 (Appendix B #12, PRD §9 decision 18); when V2 lands, the wrapper slots in between the view layer and `serverCalls.js` without any other diagram changes.
...
sequenceDiagram
    ...
    Main->>Main: read ic.definitions.cache (sync)<br/>+ load data/definitions.hero-images.json
    ...
    alt No credentials in localStorage
        ...
        Main->>State: refreshAccount({getDefinitions, applyDefinitions})
        ...
        opt instance_id available
            State->>SC: getDefinitions()
            SC->>Play: POST /getdefinitions<br/>(filter=hero_defines,attack_defines,<br/>legendary_effect_defines,campaign_defines)
            Play-->>SC: {hero_defines:[...], ...}
            SC-->>State: defs body
            State->>State: applyDefinitionsResponse(body)<br/>→ parseDefinitions → set ic.definitions.cache
            Note over State: failures swallowed,<br/>account state still committed
        end

        State-->>Main: done
        Main->>Browser: render routed view
        Note over Browser,State: views subscribe to ic.userdetails<br/>and ic.definitions.cache; re-render on either
    end
```
...
flowchart LR
    ...
    subgraph Cached["Cached (localStorage / static)"]
        DefsCache[["state.ic.definitions.cache<br/>{ heroes, effects, scopes, favors }"]]
        HeroImg[("data/definitions.hero-images.json")]
    end
    ...
    API -- "getdefinitions response" --> Parse
    Parse -- "trimmed defs<br/>(heroes, effects, scopes, favors)" --> DefsCache

    Userdetails -- "details.heroes,<br/>legendary_items" --> Classify
    DefsCache -- "heroes (pool, tags, scores)" --> Classify
    DefsCache -- "effects, scopes" --> Classify
    ...
    classDef pure fill:#e6f7e6,stroke:#2a7a2a,color:#0a3a0a
    class Classify,BuildF,BuildR,Parse pure
```

**Memoization.** `classifySlots` is the most expensive step in the pipeline and the one with the loosest natural cache key. The session-level memo key is `(selected_dps_id, ic.userdetails identity, ic.definitions.cache identity)` — any refresh of `ic.userdetails` *or* of the runtime defs cache (post-mutation or manual) invalidates the memo, and any DPS change produces a fresh entry. Because `classifySlots` is pure, the memo lives in view code (not inside the pure module) and is a plain `Map` whose entries are dropped whenever either `state.subscribe('ic.userdetails', …)` or `state.subscribe('ic.definitions.cache', …)` fires (Appendix B #14).
...
Not applicable in the conventional sense. This system has no server-side caching layer; "cache" here is `localStorage` plus a single bundled hero-portrait JSON. Three layers, each fixed by higher-priority constraints:

- **`localStorage` as source of truth for account state** — dictated by Appendix B #2 (user-driven freshness, no TTL, no auto-expiry). Evicted only on explicit Refresh or post-mutation `refreshAccount()` call.
- **`localStorage` as runtime cache for game definitions** — `ic.definitions.cache` is rewritten on every Refresh by `applyDefinitionsResponse(body)` (Appendix B #14). The `getdefinitions` call always requests the full filtered payload (no `checksum` parameter) — see [server-calls.md → "Calling getDefinitions"](./server-calls.md#calling-getdefinitions) for the rationale.
- **Bundled `data/definitions.hero-images.json`** — the only static data file. Refreshed manually via `npm run refresh-hero-images` (PRD §4.4); the script is needed because slug resolution requires CORS-blocked wiki-directory scraping that browsers can't perform. New champions render with a monogram fallback until the next maintainer push (Appendix B #9).
...
| **Scope** | The machine-readable targeting rule derived from an effect description, e.g. `{kind: 'race', value: 'human'}` or `{kind: 'stat_threshold', stat: 'int', min: 15}`. Derived at runtime by `lib/legendaryDefsParser.js` from the `getdefinitions` response and cached in `ic.definitions.cache`; consumed at runtime by `lib/scopeMatcher.js`. |
...
| 9 | **Hero tile images — source & delivery** — tiles and the DPS dropdown want a portrait for every owned hero. Where do those portraits come from, and how are they delivered at runtime? | Resolved | **Use Emmote's `ic_wiki` portraits via `<img src>` to GitHub Pages** (MIT-licensed). Rationale: Emmote's repo publishes a directory-per-hero with a `portrait.png` at a stable slug-based URL (`https://emmotes.github.io/ic_wiki/images/<slug>/portraits/portrait.png`), auto-cached by the browser's HTTP cache, zero impact on the site-shell bundle weight, and matches existing in-community attribution norms. `scripts/refresh-hero-images.js` does the hero-id → slug mapping at maintainer time (heuristics over hero names with ligature/diacritic normalization plus a small `HERO_SLUG_OVERRIDES` table; current run maps 161/173 heroes with the 12 unmapped being known placeholders) and emits `data/definitions.hero-images.json` — the only bundled data file in the repo (per Appendix B #14). At runtime `js/lib/heroImage.js` composes the URL via `heroPortraitUrl(heroId, images)` and falls back to a `heroMonogram(name)` initial-pair when a slug is missing — which is also the user-facing first-load experience for any newly-released champion until the next maintainer push. **Supporting changes that landed with this decision:** `index.html` CSP `img-src` extended to `https://emmotes.github.io`; footer gains an attribution line crediting Emmote's IC Wiki. **Fallback path considered and rejected for V1:** build-time bundling (copying images into `img/heroes/`) — rejected because the ~170 PNGs would balloon the repo size and require licensing re-checks on every hero addition, for no runtime benefit over the HTTP cache. Kept on the table as a V2 hardening move if Emmote's GitHub Pages ever becomes unreachable. |
...
| 14 | **Runtime definitions caching — what gets bundled, what gets fetched live, how is the `getdefinitions` payload trimmed and cached?** | Resolved 2026-05-05 | **Cache-only, no checksum, no deltas.** Game definitions (`hero_defines`, `attack_defines`, `legendary_effect_defines`, `campaign_defines`) are fetched at runtime via `getdefinitions` on every Refresh and parsed into the trimmed shapes the runtime consumes. The parsed result lands in `localStorage` under `ic.definitions.cache = { heroes, effects, scopes, favors, cached_at }`. **Fetch trigger:** piggybacks on the existing Refresh button — `refreshAccount()` calls `getuserdetails` first, then (if `instance_id` is available) `getdefinitions` (filter `hero_defines,attack_defines,legendary_effect_defines,campaign_defines`, no `checksum`). A `getdefinitions` failure is silent — never blocks the user-state refresh. **Why no checksum:** the payload is small enough that always-full transfer is cheaper than the bookkeeping of partial deltas, and full payloads eliminate any divergence risk between server state and client cache. See [server-calls.md → "Calling getDefinitions"](./server-calls.md#calling-getdefinitions) for the wire-level rationale. **Why parse at runtime:** the same `js/lib/legendaryDefsParser.js` module powers both the runtime cache and the offline `npm run refresh-hero-images` audit — one parser, one source of truth for the trimmed shape. **Bundled file inventory:** the only repo-committed data file is `data/definitions.hero-images.json` (~3.5 KB, hero portrait slug map). Slug resolution requires scraping Emmote's wiki directory listing — CORS-blocked from the browser — so it stays a maintainer-only refresh path (Appendix B #9). **First-load empty-cache UX:** brand-new browsers render the Legendary view with a "Refresh to load definitions" loader; a `state.subscribe('ic.definitions.cache', …)` callback in the Legendary view re-renders the moment `applyDefinitionsResponse()` writes the cache (FR-7). **Memo invalidation:** the same subscription clears the `classifySlots` view-layer memo (Appendix B #10). **Dependency injection:** `state.refreshAccount(deps)` accepts `getDefinitions` and `applyDefinitions` as injected dependencies — wired from `js/main.js` rather than imported directly inside `state.js` — to break a potential `state.js ↔ definitions.js` circular import. **Rejected alternatives:** (a) keep parsed defs bundled as `data/definitions.{heroes,legendary-effects,legendary-effect-scopes,favors}.json` — works but every game release silently goes stale until a maintainer push, and the ~85 KB bundle was a non-trivial fraction of the site shell; (b) checksum-based delta merge — was specced and rejected: more complex, no measurable wire-size benefit at this scale, and divergence risks with no automated reconciliation; (c) IndexedDB instead of `localStorage` — the parsed cache fits comfortably under the 5 MB `localStorage` budget and the synchronous read API is what the bootstrap sequence wants. |
